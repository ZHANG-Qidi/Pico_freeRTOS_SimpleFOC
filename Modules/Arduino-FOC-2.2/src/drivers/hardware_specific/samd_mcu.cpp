...
int getPermutationNumber(int pins) {
    int num = 1;
    for (int i = 0; i < pins; i++) num *= NUM_PIO_TIMER_PERIPHERALS;
    return num;
}
...
bool checkPeripheralPermutationSameTCC6(tccConfiguration& pinAh, tccConfiguration& pinAl, tccConfiguration& pinBh, tccConfiguration& pinBl, tccConfiguration& pinCh,
    ...
    if (pinAl.tcc.chan == pinBh.tcc.chan || pinAl.tcc.chan == pinBl.tcc.chan || pinAl.tcc.chan == pinCh.tcc.chan || pinAl.tcc.chan == pinCl.tcc.chan) return false;
    if (pinBl.tcc.chan == pinCh.tcc.chan || pinBl.tcc.chan == pinCl.tcc.chan) return false;

    if (pinAh.tcc.chan != pinAl.tcc.chan || pinBh.tcc.chan != pinBl.tcc.chan || pinCh.tcc.chan != pinCl.tcc.chan) return false;
    if (pinAh.wo == pinAl.wo || pinBh.wo == pinBl.wo || pinCh.wo != pinCl.wo) return false;
...
bool checkPeripheralPermutationCompatible(tccConfiguration pins[], uint8_t num) {
    for (int i = 0; i < num; i++)
        if (pins[i].tcc.tccn < 0) return false;
    for (int i = 0; i < num - 1; i++)
        for (int j = i + 1; j < num; j++)
            if (pins[i].tcc.chaninfo == pins[j].tcc.chaninfo) return false;
    for (int i = 0; i < num; i++)
        if (inUse(pins[i])) return false;
    return true;
}
...
bool checkPeripheralPermutationCompatible6(tccConfiguration& pinAh, tccConfiguration& pinAl, tccConfiguration& pinBh, tccConfiguration& pinBl, tccConfiguration& pinCh,
    ...
    if (pinAh.tcc.tccn != pinAl.tcc.tccn || pinBh.tcc.tccn != pinBl.tcc.tccn || pinCh.tcc.tccn != pinCl.tcc.tccn) return false;
...
int checkHardware6PWM(const int pinA_h, const int pinA_l, const int pinB_h, const int pinB_l, const int pinC_h, const int pinC_l) {
    for (int i = 0; i < 64; i++) {
        ...
        if (checkPeripheralPermutationSameTCC6(pinAh, pinAl, pinBh, pinBl, pinCh, pinCl)) return i;
    }
    return -1;
}
...
int checkSoftware6PWM(const int pinA_h, const int pinA_l, const int pinB_h, const int pinB_l, const int pinC_h, const int pinC_l) {
    for (int i = 0; i < 64; i++) {
        ...
        tccConfiguration pinCh = getTCCChannelNr(pinC_h, getPeripheralOfPermutation(i, 4));
        tccConfiguration pinCl = getTCCChannelNr(pinC_l, getPeripheralOfPermutation(i, 5));
        if (checkPeripheralPermutationCompatible6(pinAh, pinAl, pinBh, pinBl, pinCh, pinCl)) return i;
    }
    return -1;
}
...
int scorePermutation(tccConfiguration pins[], uint8_t num) {
    uint32_t usedtccs = 0;
    for (int i = 0; i < num; i++) usedtccs |= (1 << pins[i].tcc.tccn);
    int score = 0;
    for (int i = 0; i < TCC_INST_NUM; i++) {
        if (usedtccs & 0x1) score += 1;
        usedtccs = usedtccs >> 1;
    }
    for (int i = 0; i < TC_INST_NUM; i++) {
        if (usedtccs & 0x1) score += (num + 1);
        usedtccs = usedtccs >> 1;
    }
    return score;
}
...
int checkPermutations(uint8_t num, int pins[], bool (*checkFunc)(tccConfiguration[], uint8_t)) {
    ...
    for (int i = 0; i < (0x1 << num); i++) {
        for (int j = 0; j < num; j++) tccConfs[j] = getTCCChannelNr(pins[j], getPeripheralOfPermutation(i, j));
...
void _configure2PWM(long pwm_frequency, const int pinA, const int pinB) {
    ...
    tccConfiguration tccConfs[2] = {getTCCChannelNr(pinA, getPeripheralOfPermutation(compatibility, 0)), getTCCChannelNr(pinB, getPeripheralOfPermutation(compatibility, 1))};
...
int _configure6PWM(long pwm_frequency, float dead_zone, const int pinA_h, const int pinA_l, const int pinB_h, const int pinB_l, const int pinC_h, const int pinC_l) {
    ...
    if ((pinAh.tcc.chaninfo != pinAl.tcc.chaninfo)) configureTCC(pinAl, pwm_frequency, true, -1.0);
    configureTCC(pinBh, pwm_frequency, false, (pinBh.tcc.chaninfo == pinBl.tcc.chaninfo) ? dead_zone : -1);
    if ((pinBh.tcc.chaninfo != pinBl.tcc.chaninfo)) configureTCC(pinBl, pwm_frequency, true, -1.0);
    configureTCC(pinCh, pwm_frequency, false, (pinCh.tcc.chaninfo == pinCl.tcc.chaninfo) ? dead_zone : -1);
    if ((pinCh.tcc.chaninfo != pinCl.tcc.chaninfo)) configureTCC(pinCl, pwm_frequency, true, -1.0);
#ifdef SIMPLEFOC_SAMD_DEBUG
    SIMPLEFOC_SAMD_DEBUG_SERIAL.println("Configured TCCs...");
#endif
    ...
    for (uint8_t pin = 0; pin < PINS_COUNT; pin++) {
        ...
        switch (pinDesc.ulPort) {
            case NOT_A_PORT:
                ...
                break;
            case PORTA:
                SIMPLEFOC_SAMD_DEBUG_SERIAL.print("  PA");
                break;
            case PORTB:
                SIMPLEFOC_SAMD_DEBUG_SERIAL.print("  PB");
                break;
            case PORTC:
                SIMPLEFOC_SAMD_DEBUG_SERIAL.print("  PC");
                break;
#if defined(_SAMD51_) || defined(_SAME51_)
            case PORTD:
                SIMPLEFOC_SAMD_DEBUG_SERIAL.print("  PD");
                break;
#endif
        }
        ...
        if (association.tccE >= 0) {
            ...
            SIMPLEFOC_SAMD_DEBUG_SERIAL.print("]");
            if (tcn < 10) SIMPLEFOC_SAMD_DEBUG_SERIAL.print(" ");
        } else
